...
Prefer an available browser connector for browser tasks. The optional `cdp_read` tool lists pages and reads selector-scoped DOM text and links from an already configured loopback browser; it requires `WCU_CDP_PORT` and Python `websockets>=15`. It does not enable debugging or change browser settings. `open_uri` dispatches explicit http(s) or Obsidian open/new URIs; Obsidian creation requires the chosen vault, note name, and content. Dispatch is not proof that a note was saved. `context_for_task` may list these surfaces with prerequisites. See [application surfaces](references/application-surfaces.md) when using them.
